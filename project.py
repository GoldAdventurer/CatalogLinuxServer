...
def fbconnect():
  ...
  ['app_secret']
  ######
  redirect_uri = json.loads(open('fb_client_secrets.json', 'r').read())['web']
  ['redirect_uri']
...
def showOneItem(cat_name, item_title):
	category = session.query(Category).filter_by(name=cat_name).one()
	item = session.query(Item).filter_by(title=item_title, category=category).one()
...
def showAllItems(cat_name):
	category = session.query(Category).filter_by(name=cat_name).one()
	categories = session.query(Category).order_by(asc(Category.name))
	items = session.query(Item).filter_by(category=category).order_by(asc(Item.title))
	...
	if 'username' not in login_session:
		return render_template('publicAllItems.html', categories=categories, \
			category = category, items=items, nb_items=nb_items)
	else:
	...
	if request.method == 'POST':
		if request.form['title'] and request.form['description'] and \
			...
			try:
				...
				print 'none'
				flash('The item is already in the catalog. Please enter another item.')
				return redirect(url_for('newItem'))
			except NoResultFound:
				print "not none"
				newItem = Item(title=title, description=description, category=category,\
				cat_id=category.id, user_id=login_session['user_id'])
				session.add(newItem)
				session.commit()
				flash('Item %s Successfully Created.' % title)
				return redirect(url_for('showAllItems', cat_name=category.name))
...
def editItem(item_title, cat_name):
	if 'username' not in login_session:
		return redirect('/login')
	category = session.query(Category).filter_by(name=cat_name).one()
	item = session.query(Item).filter_by(title=item_title, cat_id=category.id).one()
	categories = session.query(Category).order_by(asc(Category.name))

	if item.user_id != login_session['user_id']:
		return "<script>function myFuction(){alert('Tou are not authorized to edit this \
      restaurant. Please create your own restaurant in order to delete.');}</script>\
	...
	if request.method == 'POST':
		...
		return render_template('editItem.html', item=item, \
			cat_name=item.category.name, categories=categories)
...
def deleteItem(cat_name, item_title):
	...
	if item.user_id != login_session['user_id']:
		return "<script>function myFuction(){alert('Tou are not authorized to \
			edit this menu item. Please create your own menu in order \
